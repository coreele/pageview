...
name: design-ui
description: >-
  Writes UI/UX design to workflow/docs/features/<feature-id>/ui-design.md (or split
...
- 输入：
  - `workflow/docs/manager/<feature-id>.md`（含 `UI 表面`）；
  - 对应切片的 Spec（Spec 门禁为 `required` 时）；
  - `<切片目录>/design.md`（若已存在）；
  - `workflow/docs/standards/ui.md`。
...
2. 读取 Spec、`workflow/docs/standards/ui.md` 与既有 `design.md`。
...
Planner 在同一切片编写 `plan.md` 时必须引用本文件（任务、验证证据、文档影响）。Developer / Reviewer / QA 在 `UI 表面` ≠ `none` 时对照本文件与 `workflow/docs/standards/ui.md`。
