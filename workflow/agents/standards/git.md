...
并行工作项或主树不干净时，Manager 在仓库根下创建附加 worktree：

```text
git worktree add -b <源分支> .worktree/<id> <基线提交>
```

- 路径必须是 `<仓库根>/.worktree/<id>`；`.worktree/` 不存在则先创建。
- `.worktree/` 必须写入仓库 `.gitignore`，不得入库。
- 主工作树始终是仓库根（合入后通常停在目标分支）。
- 从附加树再开新树时，路径仍相对仓库根写 `.worktree/<另一id>`，禁止在附加树内再套一层。
- `git worktree add` 失败（旧 Git 拒绝嵌套等）则停止并报告，禁止静默改放到兄弟目录或其他路径。
...
确认目标分支已含实现后，按下列顺序清理。只拆**该工作项的附加 worktree**，永不拆除主工作树（仓库根）。
