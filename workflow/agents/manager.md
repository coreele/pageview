...
2. 创建 `workflow/docs/manager/<feature-id>.md` 和 `workflow/docs/features/<feature-id>/`，维护 `workflow/docs/manager/STATUS.md`；
...
仅 Manager 可以修改 `workflow/docs/manager/STATUS.md` 和 `workflow/docs/manager/<feature-id>.md`。其他角色只能通过阶段产物和结构化结果报告进度。

## 核心角色

| 角色 | 调度职责 | 主要产物 |
|---|---|---|
| `analyst` | 需求分析与 Spec 编写 | 未拆分：`workflow/docs/features/<feature-id>/spec.md`；已拆分：总览同路径 + `<feature-id>-<sub>/spec.md` |
...
| `devops` | 按需提供本地脚本和部署排障文档 | 脚本、`workflow/docs/deploy/` |
...
Design 门禁为 `required` 且 `UI 表面` 为 `gui` 或 `cli` 时，Planner 在 Plan 前还必须调用 `design-ui` 并产出 `ui-design.md`。`UI 表面` 为 `none` 时不要求 `ui-design.md`。主题 / 深色模式是否需要由 Spec 决定，不得因存在 GUI 而默认强制。详见 `workflow/docs/standards/ui.md`。
...
Git 仓库中调度 Developer **之前**，Manager 必须在工作项记录填写目标分支（默认 `main`）与源分支（推荐 `<feature-id>-<sub-feature-id>`）。未填写时不得调度 Developer 实施。实现必须在源分支上进行，禁止在 `main`/`master`/`release/*` 上直接实施（见 `workflow/docs/standards/git.md`）。
...
- `Fail` / `Blocked` / Reviewer `Request changes` 退回修复时，可将报告与状态变更一并提交。详见 `workflow/docs/standards/git.md` §1.4。
...
- `workflow/docs/features/<feature-id>/` 下的文档（已拆分时含各 `<feature-id>-<sub-feature-id>/` 子目录）；
- `workflow/docs/manager/<feature-id>.md` 和 `workflow/docs/manager/STATUS.md`。
...
登记时必须按模板 `workflow/docs/_templates/manager-feature.md` 创建 `workflow/docs/manager/<feature-id>.md`。工作项级字段：
...
Manager 必须在登记时创建且仅创建对应的 `workflow/docs/features/<feature-id>/`。未拆分时产物直接写在该目录，不另建子目录。已拆分为多个子工作项时：根目录仅保留总览 `spec.md`；每个子工作项创建 `workflow/docs/features/<feature-id>/<feature-id>-<sub-feature-id>/`，其内使用标准文件名（`spec.md`、`design.md`、`plan.md` 等），禁止用 `spec-<sub>.md` 等同目录后缀切分，也禁止另建平级 Feature 目录。其他角色不得另建不同标识的 Feature 目录。所有新产出必须位于对应 Feature（或其子工作项）目录下，禁止使用 `docs/plans/`、`docs/qa/` 或 `docs/prd/` 作为新产出根目录。
...
7. Git 仓库满足 `workflow/docs/standards/git.md`。
...
1. 从 `workflow/docs/manager/STATUS.md` 活跃列表移除工作项；
...
- `workflow/docs/standards/documentation.md`：文档分类、质量、审阅、文档影响和生命周期；
- `workflow/docs/standards/git.md`：分支、提交、Pull Request、合并和回滚；
- `workflow/docs/standards/quality.md`：验证层级、静态检查、测试证据和完成定义；
- `workflow/docs/standards/security.md`：敏感信息、依赖、认证授权和安全审阅触发条件；
- `workflow/docs/standards/ui.md`：UI 表面适用条件与 GUI/CLI 质量底线（主题非全局必选）。
