...
| 对照 `workflow/docs/standards/ui.md` | 通过（代码级） | 状态完整（初始/加载/空/成功/错误/部分失败均有呈现）；错误为 `code: message + Next:`；分段控件 aria-pressed、role=group、:focus-visible 沿用既有；警示条 warning 色非 error-panel |
...
- `workflow/docs/features/index-viewer/dev-notes.md` 追加「DEF-3 修复回执」17 行，无既有内容改动。
- 无其它文件、无越界变更、无测试触碰。
