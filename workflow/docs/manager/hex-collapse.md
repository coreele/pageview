...
源分支: hex-collapse
目标分支: main
文档影响: 已归档至 `workflow/docs/archive/2026/hex-collapse/`（原 `workflow/docs/features/hex-collapse/`：spec.md、plan.md、dev-notes.md、review.md、qa-report.md）；含滚动条主题色样式合入。无独立 Design。

> 权威工作流、门禁与状态说明见 [workflow/README.md](../../README.md)。
> 活跃状态见 [STATUS.md](STATUS.md)。
>
> 文档路径（已归档）：`workflow/docs/archive/2026/hex-collapse/`。
...
- 前序 UI 合同参考归档：`workflow/docs/archive/2026/layout-chrome-split/`（含整栏 Collapse/Show hex）、`page-diagram-32b`、`pg-page-viewer`。
...
用户答复「ok」= **批准** `workflow/docs/features/hex-collapse/spec.md` 修订稿全文，包括：始终折叠、移除 Expand/Collapse free space 控件、选中仅高亮断裂带、覆盖前序 Q5、保留双向高亮/自动滚动/顶栏 Collapse hex/不改 page-core。
...
用户答复「ok」= **批准** `workflow/docs/features/hex-collapse/plan.md` 全文，按 T1–T9 实施。
...
- 2026-07-27：Analyst 产出 `workflow/docs/features/hex-collapse/spec.md`（10×P0；开放 Q1 hex 侧控件、Q2 选中是否自动展开）。状态 `speccing` → `awaiting-spec-approval`。确认前不调度 Planner。
...
- 2026-07-27：Planner 产出 `workflow/docs/features/hex-collapse/plan.md`（T1–T9）。状态 `planning` → `awaiting-plan-approval`。确认前不调度 Developer。
...
- 2026-07-27：Merge Executor FF 合入 `main`（`a2f460e`）；`git push origin main` 成功。Manager 归档至 `workflow/docs/archive/2026/hex-collapse/`。
