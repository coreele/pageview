...
| 工作树 | 实现以 commits 为准；未入库：`review.md`（Approve）、本报告、`workflow/docs/manager/**`、`.tmp-uicheck/` |
...
质量条件已满足请求合并授权前提；**本报告不提交**（`git.md` §1.4）。用户授权后由 Manager 置 `done` 并与 `review.md`/本报告一次提交，再合入 `main`/push。本轮不 merge、不 push、不改 `workflow/docs/manager/**`。
