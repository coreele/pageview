...
- 依据 Spec: workflow/docs/features/page-diagram-32b/spec.md
- 依据 Design: workflow/docs/features/page-diagram-32b/design.md
- 依据 UI Design: workflow/docs/features/page-diagram-32b/ui-design.md（`UI 表面: gui`）
- 路径等级: full
...
- 触碰: `README.md`（若页视图说明仍写 16B/列表则更新）、`workflow/docs/features/page-diagram-32b/dev-notes.md`
- 完成条件: README 用户可见说明与 32B 结构图一致（若无需改则 `dev-notes` 写 N/A 理由）；`dev-notes` 记录验证命令结果、偏离、P1-3 状态

### T10 — 收编工作区未提交的视觉优化改动（增量前置）

- 触碰: `apps/web/src/StructureMap.tsx`、`apps/web/src/HexDump.tsx`、`apps/web/src/styles.css`、`packages/page-core/src/structure-fields.ts`（既有未提交改动）；`workflow/docs/features/page-diagram-32b/dev-notes.md`
- 完成条件:
...
- 触碰: `workflow/docs/features/page-diagram-32b/dev-notes.md`；`README.md`（仅当用户可见说明需覆盖格内值/自动定位时）
...
| Server | **不改** |
| 文档 | `workflow/docs/features/page-diagram-32b/dev-notes.md`；可能 `README.md` |
...
| 类别 | 更新路径或 N/A 理由 |
|---|---|
| 开发文档 | `workflow/docs/features/page-diagram-32b/dev-notes.md`（必填；含 T10 收编说明与增量落实点）；`packages/page-core` 新导出（`valueText`、`cellCapacityChars`、`chooseCellContent`、`computeHexScrollTarget`）在包内简短注释 |
