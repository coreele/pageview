...
>
> **前序基线（不得无故回退）**：
> - `workflow/archive/2026/pg-page-viewer/spec.md`：连接与取页、空洞压缩、hex 联动等。
> - `workflow/archive/2026/page-diagram-32b/spec.md`：32B 结构图、free 折叠（Q5）、双向高亮、hex 自动定位等。
> - `workflow/archive/2026/layout-chrome-split/spec.md`：宽屏分栏、顶栏「Collapse hex」整栏显隐唯一入口、窄屏回退等。
>
> **本项显式修订（覆盖前序）**：
...
参考截图（登记引用，**非像素稿**）：`workflow/archive/2026/hex-collapse/main.md` 所引 2026-07-27 UI 截图。
...
**交接提示（Manager）**：修订稿 `workflow/archive/2026/hex-collapse/spec.md`。建议状态 **`awaiting-spec-approval`**。用户确认修订稿后方可调度 Planner。Analyst 不改 STATUS/工作项记录；不写 Design/Plan/代码；不 commit。
