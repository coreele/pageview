...
| 对照 `workflow/agents/standards/ui.md` | 通过 | 状态完整（loading/空/成功/错误/部分失败按 ui-design 状态表）；降级文案含原因+code；P1 值行为 `<button>`（Tab/Enter/Space、`:focus-visible` 高对比 outline）；复用既有 CSS 变量不新造色板 |
