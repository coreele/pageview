...
- **点表名**：未选中时选中该表并展开；`blocks > 0` 且尚未在看该表堆页时 **Load blk 0**。已选中且块列表已展开时再点表名则**收起**（不 Load、不取消选中）。已选中但已收起时再点则只展开。`blocks === 0` 时不请求页，主区提示空表。
...
- 已展开的当前表再点表名即收起，不必只点箭头。
- 宽屏树 / 结构图 / hex 之间可拖动调宽，栏间有可见间隔。
