...
1. **catalog 纯函数**（完成条件：`treeChromeVisible` 对 index 无页为 true；`visibleIndexCatalog` 按传入列表生成索引节点，展开 btree 后挂 `visibleTree` 子行；过滤空列表 hint；`indexNameClickCollapses`；段折叠 getter/toggle；`pendingFetches` 在 index 段折叠时不拉页）
...
| P0-2 | index 段按表过滤 | catalog 只含传入的过滤列表；App 用 `filterIndexesByTable` | |
...
| P1 | 段 overflow、再点收起、未选表列全部、ellipsis | CSS max-height 50%；`indexNameClickCollapses` | |
