...
- `docs/standards/ui.md`：`UI 表面` 非 `none` 时遵守对应章节；
...
存在 Spec 时，以 Spec 的合同和验收条件判定实现是否正确，并按 Plan 执行任务与验证；不存在 Spec 时，以 Plan 的范围、完成条件和验证要求为准。Design 不得替代 Spec 或 Plan。存在 `ui-design.md` 时，界面实现须与之及 `docs/standards/ui.md` 一致；禁止在 Spec / `ui-design.md` 未要求时擅自将多主题或深色模式当作必做项。
