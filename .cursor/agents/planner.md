...
调度主键为 `(feature-id, sub-feature-id)`。未拆分时二者相同，产物写在 `docs/features/<feature-id>/`；已拆分时按切片编写 Design 与 Plan，写在 `docs/features/<feature-id>/<feature-id>-<sub-feature-id>/`。
...
- 输入：
  ...
  - 对应切片的 Spec（Spec 门禁为 `required` 时）：`<切片目录>/spec.md`（必要时并读总览 `docs/features/<feature-id>/spec.md`）；
...
`design-ui` 处理信息架构、流程、状态、表面专属体验设计，以及与 Spec 验收的映射。主题 / 深色仅当 Spec 要求时写入。发现 Spec 缺失必要界面合同（如须展示的元信息、是否多主题）时，停止并报告 Manager，不得在 `ui-design.md` 中替 Spec 拍板。须遵守 `docs/standards/ui.md`。

## Plan 要求

使用 `docs/_templates/plan.md` 在切片目录编写 `plan.md`。Plan 必须包含：
...
- 禁止执行合并；
- 禁止使用 `docs/plans/`、`docs/qa/`、`docs/prd/` 等扁平目录作为新产出根；
- 禁止创建 `*-vN.md` 版本文件；
- 禁止擅自修改 `feature-id`；
