...
- 依据 Spec: `docs/features/infomask-detail/spec.md`（**approved**，2026-07-27 用户「ok」）
...
- **完成条件**:
  - 新增 `docs/features/infomask-detail/dev-notes.md`（偏离、验证命令输出摘要、手测勾选、限制）。
  - 跑下方验证命令；手测清单完成或按 quality.md §6 记 Blocked（原因/风险/恢复条件）。
  - **不**改 `docs/manager/STATUS.md` 或工作项记录；交 Review。
  - README：若无用户可见误导表述则 **N/A**（本项为详情面板呈现，当前 README 未描述 ○/● 清单）；若发现误导再改并记入 dev-notes。
- **触碰路径**: `docs/features/infomask-detail/dev-notes.md`；必要时 `README.md`
- **验收映射**: 全部 P0 + P1；文档影响
...
| `docs/features/infomask-detail/dev-notes.md` | **新增** |
...
| 类别 | 更新路径或 N/A |
|---|---|
| 开发文档 | `docs/features/infomask-detail/dev-notes.md`（必做）；实现中必要旁注仅非显而易见处 |
