...
目标分支: main
文档影响: 可选修订 README「Run」前置步骤（若修复依赖 build 或 alias）；以实际改动为准。无新 Spec/Design。

> 权威工作流、门禁与状态说明见 [docs/README.md](../README.md)。
> 活跃状态见 [STATUS.md](STATUS.md)。
>
> 文档路径：`docs/features/fix-dev-web-page-core/`（未拆分）。

## 切片（未拆分，sub-feature-id = feature-id）

| sub-feature-id | Spec | Spec 门禁 | Spec 用户确认 | Design 门禁 | UI 表面 | Review 门禁 | 状态 | 后续步骤 |
|---|---|---|---|---|---|---|---|---|
| fix-dev-web-page-core | N/A | skipped（理由：范围明确的启动缺陷；无新业务合同） | not-required | skipped（理由：无模块边界/技术选型决策；局部包解析/Vite 配置） | none | skipped（理由：fast；局部工具链修复） | done | 用户已授权合并；待 Merge Executor 合入 main |
